...
void HistogramFit_pp7TeV_Aliphysics(
				    TString infile, // = "AntiLambda-pp7TeV-Preliminary.root", //"K0s-pp7TeV-Preliminary.root",
				    TString suffix, // = "AntiLambda", //particle label
				    Double_t rangefitMin, // = 0.4, //  0.0001, //lower boundary of fit range
				    Double_t rangefitMax, // = 9.,  // 10.0,//upper boundary of fit range
			 ...
			 ) 
{
  ...
  gROOT->LoadMacro("SpectraUtils.C");
  ...
  if (suffix.Contains("Sigma0")) PDG = 3212; //Sigma+ =3222, Sigma- =3112 
  ...
  /* if( 1<0 &&  suffix.Contains("Lambda") || suffix.Contains("AntiLambda") {
  ...
  */
  printf(" ----2----- \n");
    ...
    if( suffix.Contains("Sigma0") )  {
      TH1D *data_stat = (TH1D*) file->Get("stat")->Clone("data_stat")  ; // Clone("data_stat");
      TH1D *data_syst = (TH1D*) file->Get("syst")->Clone("data_syst") ; // Clone("data_syst");
    }

    printf(" ----3----- ranges min %f max %f  \n", rangefitMin, rangefitMax );
  ...
  myDummy->GetXaxis()->SetTitle(" p_{T} (GeV/c)");
  myDummy->GetYaxis()->SetTitle(" 1/N_{evt}*d^{2}N/dydp_{T} (GeV/c)^{-1}");   	
  ...
  dummyResult->AddText(Form("  C     = %6.4f #pm %6.4f", myDummyResult->Parameter(2), myDummyResult->ParError(2) ));
...
//ratio of data points to fitted function
  ...
  //  c1->GetXaxis()->SetTitle(" p_{T} (GeV/c)");
  // c1->GetYaxis()->SetTitle(" 1/N_{evt}*d^{2}N/dydp_{T} (GeV/c)^{-1}");  
  ...
  data_stat->GetXaxis()->SetRangeUser(0.0,12.0);
