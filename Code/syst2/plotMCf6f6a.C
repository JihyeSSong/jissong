void plotMCf6f6a()
{
  ...
  TH1F *boxE = new TH1F("boxE","Pythia(red) vs Df=Phojet+Pythia(blue)",300,0.,30.);
  ...
  // Phojet+Pythia
  TFile * f1 = new TFile("Sigma0AP-p2-50-2010-Pt-bins2004-15jul15.root");

  // Phojet
  //  TFile * f1 = new TFile("Sigma0AP-p2-50-2010-Pt-f6corr-bins2004-15jul15.root");
  //  TGraphErrors  *stat1 = (TGraphErrors*)f1->Get("hTotal");
  TGraphErrors  *stat1 = (TGraphErrors*)f1->Get("data_stat");


  //Pythia
  //  TFile * f2 = new TFile("Sigma0AP-p2-50-2010-Pt-f6a-corr-bins2004-15jul15.root");
  TFile * f2 = new TFile("Sigma0AP-p2-50-2010-Pt-f6corr-bins2004-15jul15.root");
  TGraphErrors  *stat2 = (TGraphErrors*)f2->Get("data_stat");
  //  TGraphErrors  *stat2 = (TGraphErrors*)f2->Get("hTotal");
  ...
  Double_t fcorr = 1.0;
    ...
    for (Int_t iPt=0; iPt< nBinPt; iPt++) {
     ...
     ereldiff[iPt] = ediff[iPt]/yPtd1 *1.e-5 ;

     relstat[iPt] =  eyPtd1 / yPtd1 ;
     erelstat[iPt] =  eyPtd1 / yPtd1  *1.e-5; 
...
const Int_t markers[]    = {kFullCircle, kFullSquare,kOpenCircle,kOpenSquare,kOpenDiamond,kOpenCross,kFullCross,kFullDiamond,kFullStar,kOpenStar};
   ...
   TH1F *boxS = new TH1F("boxS","Relative Variation, Stat: <0.104 +- 0.038>",300,0.,30.);
  ...
  gStyle->SetOptFit(0); 
   gStyle->SetOptStat(0); 
  hRelDiff ->SetMarkerColor(kBlue);
  hRelDiff->SetLineColor(kBlue);  
  hRelDiff->SetMarkerStyle(21);
  hRelDiff->SetMarkerSize(1.8);
  hRelDiff->Draw("psame");

  hRelStat->SetMarkerStyle(20);
  hRelStat ->SetMarkerSize(1.2);
  hRelStat ->SetMarkerColor(kBlack);
  hRelStat->SetLineColor(kBlack);
  hRelStat->Draw("psame");

  TLegend *legl = new TLegend(0.5,0.5,0.9,0.9); // TLegend( X0, Y0, X1, Y1 )
    legl->AddEntry(hRelDiff,"fitted relative difference","lp");
    legl->AddEntry(hRelStat,"relative stat. uncertainty","lp");
    legl->Draw("same");
