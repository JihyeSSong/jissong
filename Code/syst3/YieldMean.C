...
YieldMean(TH1 *hstat, TH1 *hsys, TF1 *f = NULL, Double_t min = 0., Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q",TString logfilename="log.root")
{
  ...
  printf("Y 1  measure the central value \n");
  ...
  do {
    ...
    Printf(" 2 Trial-: %d", trials++);
    if(trials > 500) {
      Printf("3 FIT DOES NOT CONVERGE IN LINE %d",__LINE__);
      break;
    }
  }
  while (fitres != 0);
  ...
  cout<<"Y 4 Fit sys+stat for " <<f->GetName()<<endl;		
  cout<<"Y 5 NDF="<<f->GetNDF()<<" Chi^2="<<f->GetChisquare()<<" Chi^2/NDF="<<f->GetChisquare()/f->GetNDF()<<endl;
  ...
  printf("Y 6 STATISTICS \n");

  TCanvas *cCanvass = new TCanvas("cCanvass");
  cCanvass->Divide(2, 1);
  ...
  printf("Y 7 measure statistical error \n");

   



  printf("Y 7a fit with stat error \n");
...
{
  printf(" find lowest edge in histo \n");
  Int_t binlo;
  Double_t lo;
