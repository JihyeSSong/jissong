...
void sysfitT(){
  ...
  gStyle->SetOptFit(111);
    ...
    */
    ...
    boxU->SetXTitle("Fit parameter: dN/dy, C, n, <p_{T}>");
    boxU->SetYTitle("Value");
	...
	*/	
	...
	yMmean[ii] = (ypol3[ii] +  ypol4[ii] +  ypol5[ii] )/3;  	
	yMmean_e[ii] = 1.e-4 ; // sqrt ( yMpol3_e[ii]*yMpol3_e[ii] +yMpol4_e[ii]*yMpol4_e[ii]+yMpol5_e[ii]*yMpol5_e[ii]); 
        printf("Devs %f %f %f yMmean  %f  ii %d \n",ypol3[ii], ypol4[ii],  ypol5[ii], yMmean[ii],  ii );
	...
	printf("Mean4 y %f %f %f %f  yMmean4 %f  ii %d \n \n",  ypol1[ii], ypol3[ii], ypol4[ii],  ypol5[ii], yMmean4[ii],  ii );
    }

    // RMS calculation
    for(int ii=1; ii<ptbin+1; ii++) {
          
      dyMean[ii]=sqrt( ( (ypol3[ii]-yMmean[ii])*(ypol3[ii]-yMmean[ii]) +
			 (ypol4[ii]-yMmean[ii])*(ypol4[ii]-yMmean[ii]) + (ypol5[ii]-yMmean[ii])*(ypol5[ii]-yMmean[ii]) )/3. )     ;
      dyMean_e[ii]=1.e-4;
     printf("Mean Devs y %f %f %f %f yMmean %f  dyMean %f ii %d  \n", ypol1[ii],ypol3[ii],ypol4[ii],ypol5[ii],yMmean[ii],dyMean[ii], ii );
      ...
      printf("RMS y %f %f %f %f yMmean4  %f dyMean4 %f ii %d  \n \n",ypol1[ii],ypol3[ii],ypol4[ii],ypol5[ii],yMmean4[ii],dyMean4[ii],ii);
    }
    ...
    for(int i=1; i<ptbin+1; i++) {
      hmeandiff->SetBinContent(i+1,dyMmean[i]);
      hmeandiff->SetBinError(i+1,dyMmean_e[i]);
      printf("hdyMean %f  bin %d \n",dyMmean[i],i);
      } */
    ...
    for(int i=1; i<ptbin+1; i++) {
      hdmeandiff->SetBinContent(i+1,dyMean[i]);
      hdmeandiff->SetBinError(i+1,dyMean_e[i]);
   ...
   TH1D *hdmean4 = new TH1D("hdmean4","Mean deviation of 4 differences",ptbin+1,ptedges);
    ...
    for(int i=1; i<ptbin+1; i++) {
      hdmean4->SetBinContent(i+1,dyMean4[i]);
      hdmean4->SetBinError(i+1,dyMean4_e[i]);
    ...
    printf("diff  done---------> \n");    
     ...
     box->SetYTitle("(Y_{i} - Y_{0})/Y_{0}");
     box->SetXTitle("Fit parameter: dN/dy, C, n, <p_{T}>");
    //  boxU->SetYTitle("Value");
