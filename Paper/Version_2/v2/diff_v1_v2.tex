...
%DIF DEL ../v1/draft_v1.tex   Sun Mar 11 15:39:15 2018
%DIF ADD ./draft_v2.tex       Tue Mar 13 15:50:57 2018
...
and a minimum-bias (MB) trigger\DIFdelbegin \DIFdel{which requires at least one hit in either SPD or the }\DIFdelend \DIFaddbegin \DIFadd{. The MB trigger required a single hit in the SPD detector or in }\DIFaddend one of the \DIFdelbegin \DIFdel{V0 detectors. }\DIFdelend \DIFaddbegin \DIFadd{two VZERO counters, 
...
The trigger selection efficiency for inelastic collisions was estimated to be 85.2 \% with a +6.2 \%and ?3 \% uncertainty.  }\DIFaddend While SPD covers $|\eta|<2.0$, the two \DIFdelbegin \DIFdel{V0 }\DIFdelend \DIFaddbegin \DIFadd{VZERO }\DIFaddend detectors, each consisting of scintillator tiles, are installed on both 
...
These \DIFdelbegin \DIFdel{\red{(What is 'these'?)} }\DIFdelend \DIFaddbegin \DIFadd{VZERO detectors }\DIFaddend are used for triggering \red{(Is it the same to $f_{inel}$, which must be same to the $MB_{OR}$ = INEL?}) 
and for rejecting beam-gas interactions \red{(What is it?)}. 
Due to the low luminosity of this \red{(Where can I find it?)} data taking a high-efficiency ( 85.2 \%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cite:ALICE2015-InclPhot-pp}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cite:NSD}}%DIFAUXCMD
}\DIFaddend )  
MB trigger (\red{It is NOT the MB, but $MB_{OR}$!}) was used. 
The contamination from beam-induced background \DIFdelbegin \DIFdel{is removed offline by using the timing information and correlations \red{(Which correlation?)} in
the V0-A}\DIFdelend \DIFaddbegin \DIFadd{was reduced  to a negligible level  with the help of the timing information ofthe VZERO counters and by a cut on the position of the primary vertex reconstructed by the SPD}\DIFaddend , \DIFdelbegin \DIFdel{V0-C and SPD detectors, }\DIFdelend as discussed in detail in
\DIFdelbegin \DIFdel{ref.}\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{cite:ALICEPerformance} \red{(Is this related to N$_{offlinetrigger}$? But how?)}.  
...
\DIFdelbegin \DIFdel{\red{Events used for the data
...
}\DIFdelend \DIFaddbegin \DIFadd{Events used for the data analysis are further required to have just one reconstructed primary vertex (PV),
...
and Time Projection Chamber (TPC), and track segments in the SPD~\mbox{%DIFAUXCMD
\cite{cite:ALICEPerformance}}%DIFAUXCMD
. MB events are selected when 
the  PV is positioned along the beam axis within $\pm$10 cm from the center of the ALICE detector. 
The vertex reconstruction efficiency is 92.8 \%~\mbox{%DIFAUXCMD
\cite{cite:ALICE2015-InclPhot-pp}}%DIFAUXCMD
.
}\DIFaddend 
...
\centering  
  ...
  \DIFdelbeginFL \DIFdelFL{\red{(\asig must be removed in the plot!)}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\red{(\asig must be removed in the plot, since you say, \sig and \asig are separately done and checked its consistency!)}}\DIFaddendFL .
  Only statistical uncertainties are shown.\DIFdelbeginFL \DIFdelFL{\red{(why? How much is the systematic uncertainties for each \pt?
  - nowhere explained even also not in \ref{subsec:systematics}.)}}\DIFdelendFL }
...
\DIFdelbegin \DIFdel{\red{The MB spectrum was normalized to the number of
...
}\DIFdelend \DIFaddbegin \DIFadd{\red{The MB spectrum was normalized to the number of
$N_{norm}$ events after applying the correction factors for trigger efficiency and event selection including primary vertex reconstruction and rejection of pileup events.
The yield of \sig as a function of \pt is after background subtraction $N^{\Sigma^0}$:
\begin{equation}
\frac{d^2N}{\rm{d}p_{\rm{T}}\rm{d}y } =   \frac{1}{N_{MB}} \frac{N^{\Sigma^0}}{\Delta y \Delta \pt}
...
\begin{equation}
\DIFdelbegin \DIFdel{\frac{1}{N_{norm}}}\DIFdelend \frac{\mathrm{d}^2N}{\mathrm{d}p_{\mathrm{T}}\mathrm{d}y} = p_{\mathrm{T}} 
\frac{\mathrm{d}N}{\mathrm{d}y} \frac{(n-1)(n-2)}{nC[nC+m_{0}(n-2)]}\left[1+
\frac{\sqrt{p_{\mathrm{T}}^2+m_{0}^{2}}-m_{0}}{nC}\right]^{-n},
\label{eqn:funclevy}
\end{equation}
where \DIFdelbegin \DIFdel{$N_{norm}$ is the number of MB events after the application of the correction factors, 
\red{(How and where is the N$_{norm}$ defined? Isn't it the N$_{INEL}$, as labeled in Fig.~\ref{fig:spectra}??)}
}\DIFdelend $m_0$ is the PDG value of $m_{\Sigma^0}$~\cite{cite:PDG}. Free parameter of the fit are the $n, C$ and \dndy, which represents 
...
\pm 0.0047_{(syst)} \pm 0.0057_{(extrap)}$ is obtained from \DIFaddbegin \DIFadd{the data for measured range (}\pt \DIFadd{$>$1.1 GeV/$c$) and 
the }\DIFaddend L\'{e}vy-Tsallis fit \DIFaddbegin \DIFadd{for unmeasured range (}\pt \DIFadd{$<$ 1.1 GeV/$c$) }\DIFaddend of Fig.~\ref{fig:spectra}.
The uncertainty (not shown \red{(why?)}) related  to the overall normalization to inelastic events (or cross section) 
is fully correlated \red{(with what?)} and it amounts to +7.3\% and -3.5 \% \DIFaddbegin \DIFadd{\red{(+6.2\% and -3.0 \%?)}
}\DIFaddend ~\cite{cite:ALICE-inelasticXsection,cite:Xi_pp}.
...
Despite of large uncertainties, \sig in low \pt is significantly suppressed relative to \lam. This \DIFdelbegin \DIFdel{suppression }\DIFdelend \DIFaddbegin \DIFadd{ratio }\DIFaddend of \sig \DIFdelbegin \DIFdel{in low }\DIFdelend \DIFaddbegin \DIFadd{to $\Lambda$
in high }\DIFaddend \pt \DIFdelbegin \DIFdel{is inordinately exaggerated }\DIFdelend in the generators \DIFaddbegin \DIFadd{is abnormally suppressed }\DIFaddend due to underestimated \sig and overestimated \lam-yields \DIFaddbegin \DIFadd{relative to
the data}\DIFaddend , as shown in the previous section \ref{subsec:pT} and \cite{cite:ALICE-LF}. 
...
measurement represents a relevant baseline for further investigation in p-Pb and Pb-Pb collisions. 
 ...
 \newpage
\DIFdelbegin %DIFDELCMD < \bibliography{mybib_v1}{}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{mybib_v2}{}
\DIFaddend \bibliographystyle{utphys}
